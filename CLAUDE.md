...
**Pocket switch position labels are inverted vs physical orientation.** In `radios/pocket/MODELS/model*.yml`, EdgeTX YAML `pos: up` corresponds to the *low* channel value (~1000) and `pos: down` to the *high* value (~2000) — the opposite of what you'd guess from the label. So a Betaflight aux range like `1500-2100` activates at the high end = YAML `pos: down`. Concretely: `SA: up` = disarmed (correct), and to require acro-mode at boot (no ANGLE on SB low, no FLIP on SB mid) you need `SB: down` (= SB physically up = high channel value). Always reason from the channel value, not the YAML label. Don't know yet whether this is a Pocket hardware-mount thing or an EdgeTX-on-Pocket firmware convention — applies to SA/SB/SC/SD; SE is a momentary toggle so it doesn't have a startup position. Unknown for other radios.
